...
describe("LendingPool", function () {
    async function setup() {
        ...
        const MockFailingContractFactory = await ethers.getContractFactory("MockFailingContract");
        const mockFailingContract = await MockFailingContractFactory.deploy();
        ...
        return { lendingPool, mockFailingContract, mockToken, mockLendingPool, priceOracle, collateralManager, borrower, interestRate, admin, user1, user2 };
    }

    it("Should set contract addresses when called by admin", async function () {
        const { lendingPool, collateralManager, priceOracle, borrower, interestRate, admin } = await loadFixture(setup);
    
        await lendingPool.connect(admin).setContractAddresses(priceOracle.getAddress(), collateralManager.getAddress(), borrower.getAddress(), interestRate.getAddress());
       
        expect(await lendingPool.interestRate()).to.equal(await interestRate.getAddress());
        ...
        expect(await lendingPool.collateralManager()).to.equal(await collateralManager.getAddress());
    });
    
    it("Should revert if called by non-admin", async function () {
        const { lendingPool, collateralManager, priceOracle, borrower, interestRate, user1 } = await loadFixture(setup);
        
        await expect(
            lendingPool.connect(user1).setContractAddresses(priceOracle.getAddress(), collateralManager.getAddress(), borrower.getAddress(), interestRate.getAddress())
        ).to.be.revertedWith("Only admin can call this function");
    });

    it("should return the correct list of allowed tokens", async function () {
        const { lendingPool, collateralManager, mockToken, admin } = await loadFixture(setup);
    ...
    it("Should deposit assets correctly", async () => {
        const { lendingPool, collateralManager, interestRate, mockToken, admin, user1 } = await loadFixture(setup);
        ...
        const lenderAsset = await lendingPool.lenderAssets(user1.address, mockToken.getAddress());
        expect(lenderAsset.amount).to.equal(depositAmount);
        expect(lenderAsset.liquidityIndex).to.equal(liquidityIndexRetrieved);

        const assetBalance = await lendingPool.assetBalances(mockToken.getAddress());
        expect(assetBalance).to.equal(depositAmount);

        const totalSupplied = await lendingPool.totalSupplied(mockToken.getAddress());
    ...
    it("Should revert if the service fee is incorrect when deposit", async function () {
        ...
        await expect(
            lendingPool.connect(user1).depositAsset(mockToken.getAddress(), depositAmount, { value: ethers.parseUnits("0.005", 18) })
    ...
    it("Should revert if amount is zero", async function () {
        ...
        await expect(lendingPool.connect(user1).depositAsset(mockToken.getAddress(), 0)
    ...
    it("Should revert if token is not allowed", async function () {
        ...
        await expect(lendingPool.connect(user1).depositAsset(invalidAddress, depositAmount)
    ...
    it("Should withdraw correctly", async function () {
        const { lendingPool, collateralManager, mockToken, interestRate, admin, user1 } = await loadFixture(setup);
        ...
        const [totalBalance, ] = await lendingPool.connect(user1).getTotalBalance(mockToken.getAddress());
        
        const withdrawAmount = ethers.parseUnits("50", 18);
        await lendingPool.connect(user1).withDraw(mockToken.getAddress(), withdrawAmount, { value: serviceFee });
    
        const lenderAsset = await lendingPool.lenderAssets(user1.address, mockToken.getAddress());
    
        const reserveData = await interestRate.getReserveData(mockToken.getAddress());
    ...
    it("should fail to withdraw when amount is zero", async function () {
        ...
        await expect(lendingPool.connect(user1).withDraw(mockToken.getAddress(), 0))
    ...
    it("should fail to withdraw when token is not allowed", async function () {
        ...
        await expect(lendingPool.connect(user1).withDraw(mockToken.getAddress(), ethers.parseUnits("50", 18)))
    ...
    it("Should revert if the service fee is incorrect when with draw", async function () {
        ...
        await expect(
            lendingPool.connect(user1).withDraw(mockToken.getAddress(), withdrawAmount, { value: ethers.parseUnits("0.05", 18) })
        ).to.be.revertedWith("Incorrect service fee amount");
    });
    
    it("Should fail to withdraw when balance is insufficient", async function () {
        const { lendingPool, collateralManager, mockToken, admin, user1 } = await loadFixture(setup);
    ...
    it("Should fail to withdraw when liquidity is insufficient", async function () {
        ...
        await expect(lendingPool.connect(user1).withDraw(mockToken.getAddress(), withdrawAmount, { value: serviceFee }))
            .to.be.revertedWith("Insufficient liquidity");
    });

    it("Should revert if a non-authorized contract tries to transfer", async function () {
        const { lendingPool, borrower, mockToken, collateralManager, interestRate, admin, user1 } = await loadFixture(setup);
  
        await expect(
            lendingPool.connect(user1).transferLoan(mockToken.getAddress(), user1.address, 1000)
        ).to.be.revertedWith("Only authorized contracts can call this function");

        await expect(
            lendingPool.connect(user1).transferExcessAmount(mockToken.getAddress(), user1.address, 1000)
        ).to.be.revertedWith("Only authorized contracts can call this function");
    });
    ...
    it("Should calculate utilization rate correctly", async function () {
        ...
        await lendingPool.connect(user1).depositAsset(mockToken.getAddress(), depositAmount, { value: serviceFee });
        const utilizationRate = await lendingPool.getCurrentUtilizationRate(mockToken.getAddress());
        const totalSupplied = ethers.parseUnits("100", 18);
        const totalBorrowed = ethers.parseUnits("0", 18);
        const expectedUtilizationRate = (totalBorrowed*BigInt(10000)) /(totalSupplied);
    ...
    it("Should allow admin to withdraw service fee", async function () {
        const { collateralManager, mockToken, lendingPool, admin, user1 } = await loadFixture(setup);
        ...
        expect(user1Balance).to.be.gt(serviceFee, "User1 does not have enough ETH to cover the service fee");
        ...
        await collateralManager.connect(user1).addCollateral(collateralAddress, amount, { value: serviceFee });

        const withdrawAmount = ethers.parseUnits("0.0005", 18); 

        const initialAdminBalance = await ethers.provider.getBalance(admin.address);
        const initialContractBalance = await ethers.provider.getBalance(lendingPool.getAddress());
        
        const transaction = await lendingPool.connect(admin).withdrawServiceFee(withdrawAmount);
        ...
        const finalContractBalance = await ethers.provider.getBalance(lendingPool.getAddress());

        expect(finalAdminBalance).to.be.equal(initialAdminBalance + (withdrawAmount) - txReceipt.gasPrice*(txReceipt.cumulativeGasUsed));
        expect(finalContractBalance).to.be.equal(initialContractBalance - (withdrawAmount));
    });
    ...
    it("Should revert when non-admin tries to withdraw service fee", async function () {
        ...
        await expect(lendingPool.connect(user1).withdrawServiceFee(withdrawAmount))
          .to.be.revertedWith("Only admin can call this function");
    });
      ...
      it("Should revert when trying to withdraw more than the service fee balance", async function () {
        ...
        await expect(lendingPool.connect(admin).withdrawServiceFee(withdrawAmount))
