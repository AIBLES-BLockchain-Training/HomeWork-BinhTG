...
describe("CollateralManager", function () {
    async function setup() {
        ...
        return { collateralManager, priceOracle, borrower, lendingPool, interestRate, mockToken, mockLendingPool, admin, user1, user2 };
      }

      it("Should set contract addresses when called by admin", async function () {
        const { collateralManager, priceOracle, borrower, lendingPool, admin } = await loadFixture(setup);
    
        await collateralManager.connect(admin).setContractAddresses(priceOracle.getAddress(), borrower.getAddress(), lendingPool.getAddress());

        expect(await collateralManager.priceOracle()).to.equal(await priceOracle.getAddress());
        expect(await collateralManager.borrower()).to.equal(await borrower.getAddress());
        expect(await collateralManager.lendingPool()).to.equal(await lendingPool.getAddress());
      });
    
      it("Should revert if called by non-admin", async function () {
        const { collateralManager, priceOracle, borrower, lendingPool, admin, user1 } = await loadFixture(setup);
        
        await expect(
          collateralManager.connect(user1).setContractAddresses(priceOracle.getAddress(), borrower.getAddress(), lendingPool.getAddress())
    ...
    it("Should revert if user add collateral and collateral is locked", async function () {
      const { borrower, lendingPool, collateralManager, priceOracle, interestRate, mockToken, admin, user1 } = await setup();
      ...
      await borrower.connect(user1).createLoan(mockToken.getAddress(), tokenAmount, collateralAddresses, { value: serviceFee });
      
      await expect(
        collateralManager.connect(user1).removeCollateral(collateralAddress, amount)
    ...
    it("Should allow users to add collateral", async function () {
      ...
      await expect(() =>
          collateralManager.connect(user1).addCollateral(collateralAddress, amount, { value: serviceFee })
      ).to.changeEtherBalance(lendingPool, serviceFee);

      expect(await collateralManager.getCollateralAmount(user1.address, collateralAddress)).to.equal(amount);
    ...
    it("Should allow users to add collateral twice time", async function () {
      ...
      expect(await collateralManager.getCollateralAmount(user1.address, collateralAddress)).to.equal(ethers.parseUnits("200", 18));
    ...
    it("Should revert if token amount is zero", async function () {
        const { collateralManager, mockToken, admin, user1 } = await loadFixture(setup);
   ...
   it("Should fail to add collateral if incorrect service fee amount", async function () {
        ...
        await expect(
            collateralManager.connect(user1).addCollateral(collateralAddress, amount, { value: ethers.parseUnits("0.005", 18) })
      ...
      it("Should allow users to remove collateral", async function () {
            ...
            await collateralManager.connect(user1).addCollateral(collateralAddress, amount, { value: serviceFee });
            await collateralManager.connect(user1).removeCollateral(collateralAddress, amount,{ value: serviceFee });
            expect(await collateralManager.getCollateralAmount(user1.address, collateralAddress)).to.equal(0);
      });
      ...
      it("Should revert if there is not enough collateral to remove", async function () {
        ...
        await expect(
            collateralManager.connect(user1).removeCollateral(collateralAddress, tooMuchAmount, { value: serviceFee })
     ...
     it("Should revert if the service fee is incorrect", async function () {
      ...
      await expect(
          collateralManager.connect(user1).removeCollateral(collateralAddress, amount, { value: ethers.parseUnits("0.005", 18) })
      ).to.be.revertedWith("Incorrect service fee amount");
      });

      it("Should revert when add collateral and transferring ETH to lendingPool fails", async function () {
        const { admin, user1, mockToken, priceOracle, borrower, collateralManager, mockLendingPool } = await loadFixture(setup);
        ...
        await expect(
            collateralManager.connect(user1).addCollateral(mockToken.getAddress(), collateralAmount, { value: serviceFee })
    ...
    it("Should revert when remove collateral and transferring ETH to lendingPool fails", async function () {
     ...
     ;
        ...
        await expect(
            collateralManager.connect(user1).removeCollateral(mockToken.getAddress(), collateralAmount, { value: serviceFee })
        ).to.be.revertedWith("Transfer of service fee failed");
    });

      it("Should calculate collateral value correctly", async function () {
          const { collateralManager, mockToken, admin, user1, priceOracle } = await loadFixture(setup);
          ...
          const collateralValue = await collateralManager.getCollateralValueForTokens(user1.address, [collateralAddress]);       //
      ...
      it("Should revert if a non-authorized contract tries to lock and unlock collaterals", async function () {
        ...
        await expect(
            collateralManager.connect(user1).lockCollaterals(user1.address, [mockToken.getAddress()])
        ).to.be.revertedWith("Only authorized contracts can call this function");

        await expect(
          collateralManager.connect(user1).unlockCollaterals(user1.address, [mockToken.getAddress()])
        ).to.be.revertedWith("Only authorized contracts can call this function");
      });
      ...
      it("Should calculate collateral value correctly for mixed allowed and non-allowed tokens", async function () {
        ...
        await collateralManager.connect(admin).setAllowedToken([allowedTokenAddress]);
        ...
        await mockToken.connect(user1).approve(collateralManager.getAddress(), amount);
        await collateralManager.connect(user1).addCollateral(allowedTokenAddress, amount, { value: serviceFee });
    
        const collateralValue = await collateralManager.getCollateralValueForTokens(user1.address, [
            allowedTokenAddress,
            disallowedTokenAddress
        ]);
    
        const totalValue = (amount * price) / BigInt(10 ** 18);
        expect(collateralValue).to.equal(totalValue, "Total value should include only allowed tokens");
    });
    ...
    it("Should revert if a non-authorized contract tries to transfer collateral", async function () {
      ...
      await expect(
        collateralManager.connect(user1).transferCollateral(mockToken.getAddress(), 1000)
      ).to.be.revertedWith("Only authorized contracts can call this function");
    });
});
