...
describe("Borrower", function () {
    ...
    it("Should set contract addresses when called by admin", async function () {
        const { borrower, collateralManager, priceOracle, lendingPool, interestRate, admin } = await loadFixture(setup);
    
        await borrower.connect(admin).setContractAddresses(priceOracle.getAddress(), collateralManager.getAddress(), lendingPool.getAddress(), interestRate.getAddress());
       
        expect(await borrower.interestRate()).to.equal(await interestRate.getAddress());
        expect(await borrower.priceOracle()).to.equal(await priceOracle.getAddress());
        expect(await borrower.lendingPool()).to.equal(await lendingPool.getAddress());
        expect(await borrower.collateralManager()).to.equal(await collateralManager.getAddress());
    });
    
    it("Should revert if called by non-admin", async function () {
        const { borrower, collateralManager, priceOracle, lendingPool, interestRate, user1 } = await loadFixture(setup);
        
        await expect(
            borrower.connect(user1).setContractAddresses(priceOracle.getAddress(), collateralManager.getAddress(), lendingPool.getAddress(), interestRate.getAddress())
    ...
    it("should set risk parameters correctly when called by admin", async function () {
        ...
        await expect(borrower.connect(admin).setRiskParameters(mockToken.getAddress(), ltv, liquidationThreshold))
    ...
    it("should fail to set risk parameters if called by non-admin", async function () {
        ...
        await expect(borrower.connect(user1).setRiskParameters(mockToken.getAddress(), ltv, liquidationThreshold))
            .to.be.revertedWith("Only admin can call this function");
    });
    ...
    it("Should fail if no collateral addresses are provided", async function () {
        ...
        await expect(
            borrower.connect(user1).createLoan(mockToken.getAddress(), ethers.parseUnits("5", 18), [], { value: serviceFee })
    ...
    it("Should fail if incorrect service fee amount is sent", async function () {
        ...
        await expect(
            borrower.connect(user1).createLoan(collateralAddress, tokenAmount, [collateralAddress], { value: wrongServiceFee })
    ...
    it("Should fail if loan amount exceeds maximum allowed amount", async function () {
        ...
        await expect(
            borrower.connect(user1).createLoan(collateralAddress, ethers.parseUnits("150", 18), collateralAddresses, { value: serviceFee })
        ).to.be.revertedWith("Loan amount must be greater than zero and less than max loan amount in tokens");
    });
    ...
    it("Should calculate borrow APR correctly when utilization rate >= optimal utilization rate", async function () {
        ...
        await interestRate.connect(admin).setInterestRateParams(mockToken.getAddress(), 500, 750, 500, 40);
        await interestRate.connect(admin).initializeReserve(mockToken.getAddress());
    ...
    it("Should revert if the lending pool does not have enough balance to transfer loan", async function () {
        ...
        await expect(
            borrower.connect(user1).createLoan(mockToken.getAddress(), tokenAmount, collateralAddresses, { value: serviceFee })        
    ...
    it("Should repay one part correctly", async function () {
        const { borrower, lendingPool, collateralManager, priceOracle, interestRate, mockToken, admin, user1, user2 } = await setup();
        ...
        const collateralAddresses = [collateralAddress];
        const tokenAmount = ethers.parseUnits("5", 18);
        await borrower.connect(user1).createLoan(collateralAddress, tokenAmount, collateralAddresses, { value: serviceFee });


        const loanId = await borrower.loanCounter(); 

        const [totalRepayment, currentVariableBorrowIndex] = await borrower.calculateTotalRepayment(loanId);

        const repayAmount = ethers.parseUnits("4", 18);
        await mockToken.connect(user1).approve(borrower.getAddress(), repayAmount);
        
        await expect(
            borrower.connect(user1).repayLoan(loanId, repayAmount, { value: serviceFee })
    ...
    it("Should repay all loan correctly", async function () {
        ...
        const transaction = await borrower.connect(user1).repayLoan(loanId, repayAmount, { value: serviceFee });
        ...
        expect(userBalanceAfter).to.equal(userBalanceBefore - repayAmount + excessAmount);
    });
    ...
    it("Should revert if the caller is not the borrower", async function () {
        ...
        await expect(
            borrower.connect(user2).repayLoan(loanId, repayAmount, { value: ethers.parseUnits("0.01", 18) })
    ...
    it("Should revert if the repayment amount is zero", async function () {
        ...
        await expect(
            borrower.connect(user1).repayLoan(loanId, repayAmount, { value: ethers.parseUnits("0.01", 18) })
    ...
    it("Should revert if the service fee amount is incorrect", async function () {
        ...
        await expect(
            borrower.connect(user1).repayLoan(loanId, repayAmount, { value: ethers.parseUnits("0.005", 18) })
        ).to.be.revertedWith("Incorrect service fee amount");
    });
    ...
    it("Should return the current variable borrow rate", async function () {
        ...
        const actualVariableBorrowRate = await borrower.getCurrentVariableBorrowRate(mockToken.getAddress());
        console.log(actualVariableBorrowRate);
        //Khoong biet kq dung sai
    });
    ...
    it("Should calculate the health factor correctly", async function () {
        ...
        const totalCollateralValueInUSD = await collateralManager.getCollateralValueForTokens(user1.address, [collateralAddress]);
        const tokenPriceInUSD = await priceOracle.getAssetPrice(mockToken.getAddress());
        const loanAmountInUSD = (totalRepayment * tokenPriceInUSD) / BigInt(1e18);
        const expectedHealthFactor = (totalCollateralValueInUSD * liquidationThreshold) / loanAmountInUSD;
    ...
    it("Should return all loan IDs", async function () {
        ...
        await borrower.connect(user1).createLoan(collateralAddress, tokenAmount2, collateralAddresses, { value: serviceFee });
    ...
    it("Should liquidate a loan correctly", async function () {
        const { borrower, lendingPool, collateralManager, priceOracle, interestRate, mockToken, admin, user1 } = await setup();
        ...
        await borrower.connect(admin).liquidateLoan(loanId);
        
        const loanIds = await borrower.getAllLoanIds();
        expect(loanIds).to.not.include(loanId);
